...
class EditProfile extends React.Component {
  ...
  onSave = async () => {
    ...
    let {namespaceInfo, imagePreview} = this.state;
    if (namespaceInfo.displayName.length == 0) {
      Toast.show('Name must be set');
      return;
    }
    ...
    this.setState({
      ...
      setTimeout(async () => {
        try {
          ...
          const { tx, fee } = await updateKeyValue(this.wallet, FALLBACK_DATA_PER_BYTE_FEE, namespaceId, key, JSON.stringify(value), serverIPFS);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.inputKey}>
          <FloatTextInput
            noBorder
            autoCorrect={false}
            value={namespaceInfo.displayName}
            underlineColorAndroid='rgba(0,0,0,0)'
            style={{fontSize:15}}
            placeholder={'Name'}
            clearButtonMode="while-editing"
            onChangeTextValue={name => {this.setState({namespaceInfo: {...namespaceInfo, displayName: name}})}}
          />
          {/*
...
export default EditProfileScreen = connect(mapStateToProps)(EditProfile);
